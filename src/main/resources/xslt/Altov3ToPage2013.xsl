...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    xmlns:alto="http://bibnum.bnf.fr/schema/alto_bnf-v2_0.xsd"
    ...
    <!-- transform *all* Shape/Polyqqgon/@POINTS => Coords/@points paired with comma -->
    <xsl:template name="writePolygonPoints">
        <xsl:param name="points" select="./String"/>
        <xsl:variable name="coordPoints">
            <xsl:variable name="tokens" select="tokenize($points,'\s')"/>
    ...
    <xsl:template match="ComposedBlock">
        ...
        <xsl:choose>
            ...
            <!-- if no ComposedBlock@TYPE present, like with Tesseract's ALTO -->
            <xsl:when test="not(@TYPE)">
                <xsl:apply-templates select="./TextBlock">
                    <xsl:with-param name="isTable" select="false()"/>
                </xsl:apply-templates>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template match="TextLine">
        ...
        <xsl:variable name="fontSize" select="//TextStyle[@ID=$font]/@FONTSIZE"/>
            <TextLine id="{concat('tl_', $seq)}" custom="readingOrder {concat(concat('{index:', $seq2), ';}')}">
                <xsl:choose>
		            <xsl:when test="count(./Shape/Polygon) > 0">
                        <xsl:call-template name="writePolygonPoints">
                            <xsl:with-param name="points" select="./Shape/Polygon/@POINTS"/>
                        </xsl:call-template>
		            </xsl:when>
                    <xsl:otherwise>
                        <xsl:call-template name="writeCoords">
                ...
                <Baseline points="{./@HPOS},{(./@VPOS+./@HEIGHT)} {(./@HPOS+./@WIDTH)},{(./@VPOS+./@HEIGHT)}"/>                               
                <!-- produce Word nodes -->
