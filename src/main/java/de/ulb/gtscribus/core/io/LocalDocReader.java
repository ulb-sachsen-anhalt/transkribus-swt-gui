...
import de.ulb.gtscribus.core.io.format.ULBPageConverter;
// this is why we need this class
import de.ulb.gtscribus.core.io.format.XmlFormat;
import de.ulb.gtscribus.core.util.ULBPageXmlUtils;
import de.ulb.gtscribus.core.util.ULBXmlUtils;
	...
	public static TrpDoc load(final String path, DocLoadConfig config, IProgressMonitor monitor) throws IOException {
		...
		for (Entry<String, File> e : pageMap.entrySet()) {
			...
			if(pageXml != null) {
				...
				XmlFormat xmlFormat = ULBXmlUtils.getXmlFormat(pageXml);
				switch(xmlFormat){
					case PAGE_2016:
					case PAGE_2017:
					case PAGE_2018:
...
//					pageXml = Page2010Converter.convert(dest, pageXml);
					...
					// ULB : enforce creation of backup folder
					pageXml = LocalDocReader.updatePageFormatSingleFile(pageXml, backupPath);
					break;
				case PAGE_2013:
					break;
				default:
...
//			}
	...
	public static File convert(File input, File output) throws IOException {
		...
		ULBPageConverter conv = new ULBPageConverter();
		conv.setTargetSchema("2013-07-15");
		logger.info("Writing conversion output to: " + output.getAbsolutePath());

		//this method may fail but throws no Exception
		conv.run(input.getAbsolutePath(), output.getAbsolutePath(), false, false, null);
...
//			docDir.setMetadata(md);
	...
	public static PcGtsType createPageFromAlto2(final String imgFileName, File altoXml, boolean preserveOcrTxtStyles, boolean preserveOcrFontFamily,
		...
		try {
			XmlFormat xmlFormat = ULBXmlUtils.getXmlFormat(altoXml);
			if (xmlFormat.equals(XmlFormat.ALTO_2)) {
				logger.info(altoXml.getAbsolutePath() + ": Transforming ALTO v2 XMLs to PAGE XML.");
				return ULBPageXmlUtils.createPcGtsTypeFromAlto(altoXml, imgFileName, preserveOcrTxtStyles, preserveOcrFontFamily, replaceBadChars);
			}
				...
				return ULBPageXmlUtils.createPcGtsTypeFromAltov3(altoXml, imgFileName, preserveOcrTxtStyles, preserveOcrFontFamily, replaceBadChars);
			}
			else if(xmlFormat.equals(XmlFormat.ALTO_4)){
				logger.info(altoXml.getAbsolutePath() + ": Transforming ALTO v4 XMLs to PAGE XML.");
				return ULBPageXmlUtils.createPcGtsTypeFromAltov4(altoXml, imgFileName, preserveOcrTxtStyles, preserveOcrFontFamily, replaceBadChars);
			}
	...
	private static PcGtsType createPageFromAbbyy(final String imgFileName, File abbyyXml, boolean preserveOcrTxtStyles,
		...
		try{
			XmlFormat xmlFormat = ULBXmlUtils.getXmlFormat(abbyyXml);
...
//	}
	...
	public static File findFile(String imgName, File dir, String extension) {
		try {
			List<Path> candidateFiles = Files
				...
				.filter(p -> StringUtils.containsIgnoreCase(p.getFileName().toString(), imgName))
	...
	public static File findDefaultPageXmlForImage(String imagePath) {
		...
		return findXml(basename, new File(folder+File.separator+LocalDocConst.PAGE_FILE_SUB_FOLDER), false, true);
	}
	 ...
	 */
	public static File findXml(String imgName, File xmlInputDir, boolean ignorePrefix, boolean allowPartialMatch) {
...
//				logger.debug(pageName + ": found image: " + img.getName());
	...
	public static class DocLoadConfig {
		protected boolean preserveOcrTxtStyles;
		protected boolean preserveOcrFontFamily;
		protected boolean replaceBadChars;
		protected boolean forceCreatePageXml;
		protected boolean enableSyncWithoutImages;
		protected boolean syncTextWithExistingPageXml = false;
		/** used to load FEP documents */
		protected boolean findFilesIgnoringPrefix = false;
		/** 
		 * XML and txt files are assigned to an image also if names match partially.
		 * Use with caution depending on your filename scheme! 88.xml will also match 8.jpg!
		 */
		protected boolean findFilesAllowingPartialMatch = false;
		/** set to true to skip finding ABBYY OCR files */
		protected boolean skipAbbyy = false; 
		/** set to true to skip findin ALTO files */
		protected boolean skipAlto = false; 
		/** set to true to skip finding txt files */
		protected boolean skipTxt = false;
		/** clear text from TextRegion's when creating PAGE-XMLs? -> can save a lot of space... */
		protected boolean clearRegionText = false;
		...
		/** A list of image filenames (including extension, excluding path) that are actually loaded - if null, all images in a folder are loaded (=default) */
		protected List<String> imgsToLoad = null;
		
		protected boolean writeDocXml = true;
		protected boolean ignoreDocXml = false;
		...
		public void addImgsToLoad(String... imgs) {
			if (imgsToLoad == null) {
			...
			// dox-xml has to be ignored when adding specific imgs to load:
			if (!CollectionUtils.isEmpty(imgsToLoad)) {
				this.writeDocXml = false;
				this.ignoreDocXml = true;
			}
		}
		
		public void clearImgsToLoad() {
			imgsToLoad = null;
		}
		
		public List<String> getImgsToLoad() {
			return imgsToLoad;
		}
