...
public class DocumentManager extends Dialog {
	...
	static final Color darkGreen = new Color(Display.getCurrent(), 0, 150, 0);
	static final Color veryDarkGreen = new Color(Display.getCurrent(), 30, 75, 12);
	static final Color moderateBlue = new Color(Display.getCurrent(), 73, 153, 199);
	static final Color verySoftBlue = new Color(Display.getCurrent(), 178, 212, 231);
	static final Color grayishBlue = new Color(Display.getCurrent(), 104, 109, 114);
	static final Color lightYellow = new Color(Display.getCurrent(), 255, 255, 200);
	static final Color vividYellow = new Color(Display.getCurrent(), 226, 205, 19);
...
//		sort.setEnabled(false);
	    ...
	    for (String s : EnumUtils.stringsArray(EditStatus.class)){
	    	if (s.contentEquals("Ground Truth") || s.contentEquals("Final") || s.contentEquals("Done") || s.contentEquals("New") ) {
	    		setAsLatest.add(s,a++);
	    	}
	    }
	    ...
	    setAsLatest.add("previous version",a++);
	...
	private void updateColors() {
		...
		for (TreeItem i : tv.getTree().getItems()) {
			...
			for (TreeItem child : i.getItems()) {
				...
				TrpTranscriptMetadata transcriptMd = page.getCurrentTranscript();
				EditStatus status = transcriptMd.getStatus();
				
				if (transcriptMd.getNrOfTranscribedLines() != null){
					if (transcriptMd.getNrOfTranscribedLines() > 0) {
						switch (status)
						{
							case DONE:
								child.setBackground(verySoftBlue);
								break;
							case FINAL:
								child.setBackground(moderateBlue);
								break;
							case GT:
								child.setBackground(darkGreen);
								break;
							default:
								child.setBackground(lightGreen);
						}
					} else if (transcriptMd.getNrOfTranscribedLines() == 0
							&& transcriptMd.getNrOfLines() > 0) {
...
//		}
	...
	private void addMenuItems4DocLevel(Menu contextMenu) {
		...
		MenuItem revert = new MenuItem(contextMenu, SWT.NONE);
		revert.setText("Revert to version(s) prior to your last (batch) job");
		revert.setToolTipText("Latest versions - derived from (batch) job - of the document swap places with their parents - use case is e.g. if layout recognition should be undone");
