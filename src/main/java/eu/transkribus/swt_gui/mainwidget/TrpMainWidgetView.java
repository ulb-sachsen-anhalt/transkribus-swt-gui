...
import eu.transkribus.swt_gui.metadata.CustomMetadataTable;
...
//
	...
	CustomMetadataTable pageMetadataEditor;
...
//		tabWidget.jobsItem.setControl(jobOverviewWidget);
		...
		pageMetadataEditor = new CustomMetadataTable(tabWidget.metadataTf, 0);
		tabWidget.pageMdItem.setControl(pageMetadataEditor);
...
//			createDefaultLineItem = otherSaveOptionstoolItem.addItem("Create default line for selected line / baseline", null, null);
		...
		loadTranscriptInTextEditor.setImage(Images.SCRIPT_ICON);
		...
		if (TrpSettings.ENABLE_LINE_EDITOR) {
			...
			showLineEditorToggle.setImage(Images.PENCIL);
