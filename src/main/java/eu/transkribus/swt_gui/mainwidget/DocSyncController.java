	...
	public void syncPAGEFilesWithLoadedDoc() {
		...
		syncFilesWithLoadedDoc(new  DocSyncWithFilesDialog(getShell(), typeOfFiles), match -> {
			...
			if (!p.isLocalFile()) {
				...
				/*
				 * status should be taken from the 'local' PAGE XML if available, otherwise it is IN_PROGRESS
				 * 
				 */
				EditStatus tsStatus = EditStatus.IN_PROGRESS;
				if (pcGtsType.getMetadata() != null && pcGtsType.getMetadata().getTranskribusMetadata() != null && pcGtsType.getMetadata().getTranskribusMetadata().getStatus() != null) {
					String tmp = pcGtsType.getMetadata().getTranskribusMetadata().getStatus();
					tsStatus = EditStatus.valueOf2(tmp);
				}
				
				conn.updateTranscript(collId, p.getDocId(), p.getPageNr(), tsStatus,
						pcGtsType, p.getPageId(), toolname, note);				
			}
			else {
				logger.error("Syncing PAGE-XML files with remote docs does not make sense - you shouldn't get here anyway...");
			}
