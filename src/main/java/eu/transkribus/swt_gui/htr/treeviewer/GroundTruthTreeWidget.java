...
/*
 * @deprecated by {@link GroundTruthPagedTreeWidget}
 */
	...
	void initListener() {
		treeViewer.getTree().addMenuDetectListener(new MenuDetectListener() {
			...
			public void menuDetected(MenuDetectEvent event) {
				...
				if(selectionData instanceof AGtDataSet<?>) {
					AGtDataSet<?> gtSet = (AGtDataSet<?>) selectionData;
					boolean isDataSetAccessible = false;
					if(gtSet.getModel() instanceof TrpHtr) {
						TrpHtr htr = (TrpHtr) gtSet.getModel();
						isDataSetAccessible = htr.getCollectionIdLink() != null 
								|| htr.getReleaseLevelValue() >= ReleaseLevel.DisclosedDataSet.getValue();
					} else if (gtSet.getModel() instanceof TrpModelMetadata) {
						isDataSetAccessible = ((TrpModelMetadata) gtSet.getModel()).isGtAccessible();
					}
