...
//	private Combo scriptType;
	...
	private CustomTagParamsComposite customTagTrainingParamsComp;
...
//		trainSizeTxt.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false));
		...
		if(enableBaseModelSelection) {
			...
			baseModelBtn = new ModelChooserButton(this, true, ModelUtil.TYPE_TEXT, getProvider());
		...
		reverseTextParamsComp = new ReverseTextParamsComposite(this, 0);
		reverseTextParamsComp.setLayoutData(new GridData(SWT.FILL, SWT.FILL, false, false, 2, 1));
		
		customTagTrainingParamsComp = new CustomTagParamsComposite(this, SWT.NONE);
		customTagTrainingParamsComp.setLayoutData(new GridData(SWT.FILL, SWT.FILL, false, false, 2, 1));
...
//		langSelection.setItems(Locale.getISOLanguages());
		...
		Button resetDefaultsBtn = new Button(this, SWT.PUSH);
		resetDefaultsBtn.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false));
...
//		advComp.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true, 2, 1));
	...
	public void setCitlabTrainingDefaults() {
		...
		if (earlyStoppingTxt!=null) {
			earlyStoppingTxt.setText(""+ DEFAULT_EARLY_STOPPING);
		}
...
//		trainSizeTxt.setText("" + CitLabHtrTrainConfig.DEFAULT_TRAIN_SIZE_PER_EPOCH);
		...
		customTagTrainingParamsComp.setDefaults();
	}
...
//		}
	...
	public CitLabHtrTrainConfig addParameters(CitLabHtrTrainConfig citlabTrainConf) {
		citlabTrainConf.getModelMetadata().setProvider(this.getProvider());
...
//		citlabTrainConf.setTrainSizePerEpoch(Integer.parseInt(trainSizeTxt.getText()));
		...
		if(baseModelBtn != null) {
			TrpModelMetadata htr = baseModelBtn.getModel();
			if (htr != null) {
				citlabTrainConf.getModelMetadata().setBaseModelId(htr.getModelId());
			} else {
				logger.debug("No base HTR selected.");
			}
		}
		
		citlabTrainConf.setTagsToOmit(tagSelectionComp.getSelectedTags());
		citlabTrainConf.setReverseTextParams(reverseTextParamsComp.getParams());
		citlabTrainConf.setCustomTagParams(customTagTrainingParamsComp.getParams());
	...
	public String getProvider() {
		return ModelUtil.PROVIDER_CITLAB_PLUS;
	}
}
