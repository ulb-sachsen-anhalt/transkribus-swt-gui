...
public class CITlabAdvancedLaConfigDialog extends ALaConfigDialog {
	...
	final static String CUSTOM_NET_NAME = "Selected trained model...";
	...
	private Group settingsGroup, rotGroup, sepGroup, groupingGroup;
	private Button groupClusterBtn, groupBoundingBoxBtn;
...
//	private Button deleteTextCheck;
	...
	public CITlabAdvancedLaConfigDialog(Shell parent, ParameterMap parameters, JobImpl jobImpl) {
	...
	protected Control createDialogArea(Composite parent) {
		...
		modelChooserBtn = new ModelChooserButton(netGroup, true, ModelUtil.TYPE_LAYOUT, ModelUtil.PROVIDER_CITLAB_BASELINES, "Selected trained model: ");
...
//		independentSettingsGrp.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true, 1, 2));
		...
		groupingGroup = new Group(settingsGroup, 0);
		groupingGroup.setLayout(new GridLayout(2, false));
		groupingGroup.setText("Region grouping");
		groupingGroup.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true, 1, 1));
		groupClusterBtn = new Button(groupingGroup, SWT.RADIO);
		groupClusterBtn.setText("Cluster lines");
		groupClusterBtn.setToolTipText("Cluster lines into text-regions");
		groupClusterBtn.setSelection(false);
		groupBoundingBoxBtn = new Button(groupingGroup, SWT.RADIO);
		groupBoundingBoxBtn.setText("Single-Bounding-Box");
		groupBoundingBoxBtn.setToolTipText("Create a single text-region with the bounding box of all lines");
		groupBoundingBoxBtn.setSelection(false);
		...
		if (jobImpl.equals(JobImpl.TranskribusLaJob)) { // hide settings group for TranskribusLaJob as they are not implemented yet
			settingsGroup.setParent(SWTUtil.dummyShell);
		}
	...
	public static boolean isConfigurable(JobImpl jobImpl) {
		return JobImpl.TranskribusLaJob.equals(jobImpl) || JobImpl.CITlabAdvancedLaJob.equals(jobImpl);
	}
	...
	private void addListeners() {
		neuralNetCombo.addModifyListener(new ModifyListener() {
			...
			public void modifyText(ModifyEvent e) {
				if (CUSTOM_NET_NAME.equals(getSelectedNet().label)) {
					modelChooserBtn.openModelChooserDialog();
				}				
				updateGui();				
			}
		});
	...
	private void loadData() {
		...
		modelList.add(1, new LaModel(CUSTOM_NET_NAME, null));
	...
	protected void storeSelectionInParameterMap() {
		LaModel laModelFromExistingPars = resolveSelectedNetFromParameters();
...
//		}
		...
		String group = groupBoundingBoxBtn.getSelection() ? LaCITlabUtils.GROUP_SCHEME_BOUNDING_BOX : LaCITlabUtils.GROUP_SCHEME_CLUSTER;
		
		if(PRESET_NET_NAME.equals(m.getLabel()) || CUSTOM_NET_NAME.equals(m.getLabel())) {
			RotScheme rot = null;
			...
			parameters.addParameter(LaCITlabUtils.GROUP_SCHEME_KEY, group);
			
			if (CUSTOM_NET_NAME.equals(m.getLabel())) {
				...
				else if (laModelFromExistingPars != null) {
					parameters.addParameter(JobConst.PROP_MODEL_ID, laModelFromExistingPars.getModelId());
					parameters.addParameter(JobConst.PROP_MODELNAME, laModelFromExistingPars.getLabel());					
				}
				else {
					throw new IllegalArgumentException("You have to specify a trained model using the model chooser button!");
				}
			}
		} 
		else {
...
//		}
		...
		String groupScheme = parameters.getParameterValue(LaCITlabUtils.GROUP_SCHEME_KEY);
		if (StringUtils.equals(groupScheme, LaCITlabUtils.GROUP_SCHEME_BOUNDING_BOX)) {
			groupBoundingBoxBtn.setSelection(true);
		}
		else {
			groupClusterBtn.setSelection(true);
		}
	...
	public String getSelectedModelInfoString() {
		LaModel model = resolveSelectedNetFromParameters();
		return NEURAL_NET_LBL + " " + (model == null ? "Preset" : model.getLabel());
	}
...
//		}
		...
		if (JobImpl.TranskribusLaJob.equals(jobImpl)) {
			return infoStr;
		}
	...
	private void updateGui() {
		...
		if (CUSTOM_NET_NAME.equals(m.label)) {
			modelChooserBtn.setVisible(true);
		}
		else {
			modelChooserBtn.setVisible(false);
		}		
	}
	...
	private void setSelectedNet(LaModel m) {
		...
		// set selected model in ModelChooserButton
		neuralNetCombo.setText(CUSTOM_NET_NAME);
		modelChooserBtn.setText(m.getLabel());
	}
	/**
	...
	private LaModel resolveSelectedNetFromParameters() {
		...
		LaModel trainedLaModel = new LaModel(netFilename, null);
		trainedLaModel.setModelId(parameters.getParameterValue(JobConst.PROP_MODEL_ID));
		return trainedLaModel;
	}
