...
//	}
	...
	public void drawOutline(SWTCanvas canvas, GC gc) {
		...
		if (isClosed()) {
			...
			/*
			 * fill structured regions or lines
			 * isStructEditorSelected()
			 */
			ITrpShapeType currShapeType = getTrpShapeType();
			if (!currShapeType.getStructure().isEmpty() ){//&& currShapeType instanceof TrpRegionType) {
				gc.setAlpha((int) (sets.getBackgroundAlpha()*0.75));
				gc.fillPolygon(pointArray);
			}
	...
	public void drawStructType(SWTCanvas canvas, GC gc) {
		...
		boolean drawStructTypeText = TrpConfig.getTrpSettings().isShowStructTypeText() && (isSelected() || canvas.getScene().structTypeMap.containsKey(st.getStructure()));
		...
		//if baseline is selected -> show struct type of parent line
		if (st instanceof TrpBaselineType) {
			ACanvasShape af = (ACanvasShape) getParent();
			af.drawStructType(canvas, gc);
		}
		
		canvas.getScene().structTypeMap.remove(st.getStructure());
		
		int fontHeight = 20;
		String structStr = st.getStructure();
		Rectangle bounds = getBounds();
		Font old = gc.getFont();
		Font font = Fonts.createFontWithHeight(gc.getFont(), fontHeight);
		gc.setFont(font);
		
		//if (isSelected()) {
			gc.setAlpha(100);
			gc.fillRectangle(new org.eclipse.swt.graphics.Rectangle(bounds.x, bounds.y, gc.stringExtent(structStr).x, -gc.stringExtent(structStr).y - 3));
			gc.setAlpha(255);
...
//	}
	...
	public Color getColor() {
		...
		boolean drawStructColor = TrpConfig.getTrpSettings().isShowStructTypeColor() && !structType.isEmpty();
		//boolean drawStructColor = isStructEditorSelected() && TrpConfig.getTrpSettings().isShowStructTypeColor() && !TrpConfig.getTrpSettings().isDrawShapesInDefaultColorsInStructEditor();
		if (drawStructColor) {
...
//	}
	...
	public Pair<ICanvasShape, ICanvasShape> splitByPolyline(CanvasPolyline pl) {
		...
		CanvasPolygon pUp = pl.extendAtEnds(extDist).getPolyRectangle(extDist, 0);
		CanvasPolygon pDown = pl.extendAtEnds(extDist).getPolyRectangle(0, extDist);
