...
import eu.transkribus.swt_gui.credits.CreditTransactionsByJobDialog;
...
public class JobTableWidgetListener extends SelectionAdapter implements IStorageListener, IDoubleClickListener, ISelectionChangedListener {
	...
	void attach() {
		...
		jw.getShowCreditTransactionsBtn().addSelectionListener(this);
	...
	void detach() {
		...
		jw.getShowCreditTransactionsBtn().removeSelectionListener(this);
	...
	@Override public void widgetSelected(SelectionEvent e) {
		...
		else if (s.equals(jw.getShowCreditTransactionsBtn())) {
			TrpJobStatus job = jw.getFirstSelected();
			if(job != null) {
				logger.debug("Opening transaction overview for job {}", job.getJobId());
				CreditTransactionsByJobDialog d = new CreditTransactionsByJobDialog(jw.getShell(), job);
				d.open();
			}
		}
	}
...
//			jw.setInput(new ArrayList<>(storage.getJobs()));
	...
	public void selectionChanged(SelectionChangedEvent arg0) {
		...
		boolean hasTransactions = jobStatus != null 
				&& storage.getCreditCosts(null, false).stream().anyMatch(c -> c.getJobImpl().equals(jobStatus.getJobImpl()));
		jw.getShowCreditTransactionsBtn().setEnabled(hasTransactions);
	}	
