...
import eu.transkribus.swt_gui.models.ModelFilterWithProviderWidget.IModelFilterWithProviderListener;

public class ServerWidgetListener extends SelectionAdapter implements Listener, ISelectionChangedListener, IDoubleClickListener, KeyListener, MouseTrackListener, IStorageListener, TraverseListener, IModelFilterWithProviderListener {
	...
	public void attach() {
		...
		sw.groundTruthTreeWidget.getFilterComposite().addListener(this);
		SWTUtil.addSelectionListener(sw.groundTruthTreeWidget.getReloadButton(), this);
...
//	}
	...
	@Override public void doubleClick(DoubleClickEvent event) {
		...
		if(sw.isCollectionsTabSelected() && el instanceof TrpDocMetadata) {
			...
			if (el instanceof TrpModelMetadata) {
				//double-click opens the models dialog showing details of this HTR
				sw.showModelDetailsDialog((TrpModelMetadata) el);
				//expand tree item
				//sw.expandGroundTruthTreeItem(el);
			} else if (el instanceof ModelGtDataSet) {
				ModelGtDataSet set = (ModelGtDataSet) el;
				if(!storage.isUserAllowedToViewDataSets(set.getModel())) {
					//no action
					return;
				}
				mw.loadGroundTruthAsDoc(set, 0);
			} else if (el instanceof ModelGtDataSetElement) {
				ModelGtDataSetElement page = (ModelGtDataSetElement)el;
				if(!storage.isUserAllowedToViewDataSets(page.getParentGtDataSet().getModel())) {
					//no action
					return;
				}
				mw.loadGroundTruthAsDoc(page.getParentGtDataSet(), page.getGroundTruthPage().getPageNr() - 1);
				
				
			//TODO remove this
			} else if (el instanceof TrpHtr) {
				//double-click opens the models dialog showing details of this HTR
	...
	public void widgetSelected(SelectionEvent e) {
		...
		if (s == sw.recentDocsComboViewerWidget.lastDocsCombo){
			...
			sw.groundTruthTreeWidget.refreshPage(false);
		}
	}
...
//		
	...
	@Override
	public void comboFiltersChanged(Combo c) {
		logger.debug("comboFiltersChanged in ServerWidget, combo text = "+c.getText());
		sw.groundTruthTreeWidget.refreshPage(false);
	}
