...
public class TagSpecsWidget extends Composite {
	...
	Button showUserTagsBtn, showCollTagsBtn, showTagSpecsBtn, customizeBtn;
	Label headerLbl;
	...
	public TagSpecsWidget(Composite parent, int style, boolean isEditable) {
		...
		if (!isEditable) {
			showUserTagsBtn = new Button(header, SWT.CHECK | SWT.FLAT);
			showUserTagsBtn.setText("User tags");
			showUserTagsBtn.setToolTipText("Show all available tags");
			showUserTagsBtn.addSelectionListener(new SelectionAdapter()
			{
				public void widgetSelected(org.eclipse.swt.events.SelectionEvent event) {
					if (showUserTagsBtn.getSelection()) {
						showCollTagsBtn.setSelection(!showUserTagsBtn.getSelection());
						
					}
					else {
						showTagSpecsBtn.setSelection(!showUserTagsBtn.getSelection() && !showCollTagsBtn.getSelection());
					}
					updateAvailableTagSpecs();
				}
				
			});
			DataBinder.get().bindBeanToWidgetSelection(TrpSettings.SHOW_ALL_TAGS_IN_TAG_EDITOR_PROPERTY, TrpConfig.getTrpSettings(), showUserTagsBtn);
			
			showCollTagsBtn = new Button(header, SWT.CHECK | SWT.FLAT);
			showCollTagsBtn.setText("Collection tags");
			showCollTagsBtn.setToolTipText("Show all available tags");
			showCollTagsBtn.addSelectionListener(new SelectionAdapter()
			{
				public void widgetSelected(org.eclipse.swt.events.SelectionEvent event) {
					if (showCollTagsBtn.getSelection()) {
						showUserTagsBtn.setSelection(!showCollTagsBtn.getSelection());
						showTagSpecsBtn.setSelection(!showCollTagsBtn.getSelection());
					}
					else {
						showTagSpecsBtn.setSelection(!showUserTagsBtn.getSelection() && !showCollTagsBtn.getSelection());
					}
			...
			showTagSpecsBtn = new Button(header, SWT.CHECK | SWT.FLAT);
			showTagSpecsBtn.setText("Tag specifications");
			showTagSpecsBtn.setToolTipText("Show specified tags");
			showTagSpecsBtn.addSelectionListener(new SelectionAdapter()
			{
				public void widgetSelected(org.eclipse.swt.events.SelectionEvent event) {
					if (showTagSpecsBtn.getSelection()) {
						showUserTagsBtn.setSelection(!showTagSpecsBtn.getSelection());
						showCollTagsBtn.setSelection(!showTagSpecsBtn.getSelection());
					}
					updateAvailableTagSpecs();
		...
		if (true) {
			...
			colorCol.setLabelProvider(new CellLabelProvider() {
				@Override public void update(ViewerCell cell) {
	                ...
	                String tagColor = tagSpec.color;
	                
	                //logger.debug("tag color for tag: "+tagSpec.getCustomTag().getTagName()+" color: "+tagColor);
	                
	                if (tagColor == null) {
	                	tagColor = CustomTagFactory.getTagColor(tagSpec.getCustomTag().getTagName());
	                	if (tagColor == null)
...
//			if (!isEditable && !showCollTagsBtn.getSelection()) {
//				showUserTagsBtn.setSelection(showAllTags);	
//			}
			
			if (isEditable || (!showUserTagsBtn.getSelection() && !showCollTagsBtn.getSelection())) {
				headerLbl.setText("Tag Specifications");
				tableViewer.setInput(Storage.getInstance().getCustomTagSpecs());
			}
			else if (showCollTagsBtn.getSelection()) {
				headerLbl.setText("Collection Tags");
				List<CustomTagSpec> allTagsSpecs = new ArrayList<>();
				for (CustomTag t : CustomTagFactory.getRegisteredTagObjectsSortedByName(true, true)) {
					...
					ts.color = CustomTagFactory.getCollectionTagColor(tc.getTagName());
					allTagsSpecs.add(ts);
				}
				...
				List<CustomTagSpec> allTagsSpecs = new ArrayList<>();
				for (CustomTag t : CustomTagFactory.getRegisteredTagObjectsSortedByName(true, false)) {
					if (!t.showInTagWidget() || t.getTagName().equals(TextStyleTag.TAG_NAME)) { // exclude TextStyle tag
						continue;
					}
...
//					}
					...
					CustomTagSpec ts = new CustomTagSpec(tc);
					ts.color = CustomTagFactory.getTagColor(tc.getTagName());
					allTagsSpecs.add(ts);
				}
				tableViewer.setInput(allTagsSpecs);
			}
