...
public class ShareModelDialog extends Dialog  {
	...
	private final TrpModelMetadata model;
	...
	public ShareModelDialog(Shell parentShell, TrpModelMetadata model) {
		super(parentShell);
		this.model = model;
		store = Storage.i();
	}
	...
	public ShareModelDialog(Shell parentShell, int modelId, String modelName) {
		this(parentShell, constructDummyModel(modelId, modelName));
		
	}
	/**
	 * @deprecated
	 */
	private static TrpModelMetadata constructDummyModel(int modelId, String modelName) {
	...
	protected void configureShell(Shell shell) {
		...
		shell.setText("Share model '" + getModel().getName() + "'");
...
//		Predicate<TrpCollection> collSelectorPredicate = (c) -> { return c!=null && AuthUtils.canManage(c.getRole()); }; // show only collections users can manage
	...
	protected TrpModelMetadata getModel() {
		return new TrpModelMetadata(model);
	}
	...
	protected void addModelToCollection(int colId)
		...
		store.getConnection().getP2PaLAModelCalls().addOrRemoveModelFromCollection(getModelId(), colId, false);
	}

	protected void removeModelFromCollection(int colId)
			throws TrpClientErrorException, TrpServerErrorException, SessionExpiredException {
		store.getConnection().getP2PaLAModelCalls().addOrRemoveModelFromCollection(getModelId(), colId, true);
	}
	...
	protected List<TrpCollection> getCollectionList()
		...
		return store.getConnection().getP2PaLAModelCalls().getModelCollections(getModelId());
	}
}
